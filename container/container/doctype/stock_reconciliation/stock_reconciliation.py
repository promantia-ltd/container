...
def get_container_data(docname):
	container_reco_doc=frappe.get_doc('Container Reconciliation',docname)
	item_code=container_reco_doc.item_code
	warehouse=container_reco_doc.warehouse
	new_containers=frappe.db.get_all("Container Reconciliation Items Add",filters={'parenttype':'Container Reconciliation','parent':docname},fields={'*'})
	old_containers=frappe.db.get_all("Container Reconciliation Items",filters={'parenttype':'Container Reconciliation','parent':docname},fields={'*'})
	old_container_list=""
	...
	return item_code,warehouse,old_container_list+new_container_list,container_reco_doc.qty_after_reconciliation

@frappe.whitelist()
def on_submit(doc,method):
	try:
		if doc.custom_container_reconciliation:
			container_reco_doc=frappe.get_doc('Container Reconciliation',doc.custom_container_reconciliation)

			item_doc=frappe.get_doc('Item',container_reco_doc.item_code)
			sr_item_doc_name=""
			container_nos=""
			container_reco_doc=frappe.get_doc('Container Reconciliation',doc.custom_container_reconciliation)
			new_containers=frappe.db.get_all("Container Reconciliation Items Add",filters={'parenttype':'Container Reconciliation','parent':doc.custom_container_reconciliation},fields={'*'})
			old_containers=frappe.db.get_all("Container Reconciliation Items",filters={'parenttype':'Container Reconciliation','parent':doc.custom_container_reconciliation},fields={'*'})
			...
			if sr_item_doc_name!="" and container_nos!="":
				sle=frappe.get_doc("Stock Ledger Entry",{"voucher_no":doc.name,"item_code":item_doc.item_code,"voucher_detail_no":sr_item_doc_name})
			...
			frappe.db.sql("UPDATE `tabSeries` SET `current` = `current` + %s WHERE `name`=%s", (add_new_sl_count,series.split(".#")[0],))
			frappe.db.commit()
