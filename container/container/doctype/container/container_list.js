frappe.listview_settings['Container'] = {
	add_fields: ["item_code", "warehouse", "expiry_date", "delivery_document_type","status","primary_available_qty"],
	get_indicator: (doc) => {
		if (doc.delivery_document_type && doc.primary_available_qty <=0) {
			return [__("Delivered"), "green", "delivery_document_type,is,set"];
		}else if (doc.delivery_document_type && doc.primary_available_qty > 0 ) {
