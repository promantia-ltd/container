...
async function processBOM(frm, w, bomQuantity) {
    const tabletransfer =  await frappe.model.with_doc("BOM", frm.doc.bom_no);
    frm.clear_table("items");
    const warehouse_list = [];
	let container_used = [];
    
    for (const detail of tabletransfer.items) {
        for (let i = 0; i < detail.no_of_inputs; i++) {
            const target_warehouse = await getTargetWarehouse(frm.doc.work_order, warehouse_list, w.wip_warehouse, detail);
            const containerDetails = await getContainerDetails(detail, w.source_warehouse, target_warehouse, bomQuantity, frm.doc.fg_completed_qty, detail.no_of_inputs, container_used, frm.doc.work_order);
			warehouse_list.push(target_warehouse)
			if (frm.doc.job_card !== undefined && frm.doc.job_card) {
				const job_card_detail = await frappe.model.with_doc("Job Card", frm.doc.job_card);
				if(detail.operation == job_card_detail.operation)
				// job card is there then we should only assign jobcard related items
				assignContainers(frm, detail, container_used, bomQuantity, containerDetails, target_warehouse)
				}
			else{
				assignContainers(frm, detail, container_used, bomQuantity, containerDetails, target_warehouse)
			}
...
function assignContainers(frm, detail, container_used, bomQuantity, containerDetails, target_warehouse){
	if(containerDetails.target || containerDetails.source){
		if (containerDetails.target && containerDetails.source && containerDetails.partially_reserved){
			if (containerDetails.target.container_no){
				// this is partially reserved at target
				addRowToForm(frm, detail, containerDetails.target, container_used, bomQuantity,"reserved_items");
			}
			if (containerDetails.source.container_no){
				// this is partially reserved at source
				addRowToForm(frm, detail, containerDetails.source, container_used, bomQuantity, "items");
			}
		}
		if(containerDetails.target && containerDetails.complete_reserved_at_target == 1){
			// this is completely reserved at target
			addRowToForm(frm, detail, containerDetails.target, container_used, bomQuantity, "items");
		}
		if (containerDetails.source && containerDetails.complete_reserved_at_target == 0 && !(containerDetails.target)){
		...
		child.item_code=detail.item_code;
		child.item_name=detail.item_name;
		child.required_qty=((detail.stock_qty/bomQuantity)*frm.doc.fg_completed_qty)/detail.no_of_inputs;
		child.qty=((detail.stock_qty/bomQuantity)*frm.doc.fg_completed_qty)/detail.no_of_inputs;
		child.basic_rate=detail.rate;
		child.uom=detail.stock_uom;
		child.conversion_factor=detail.conversion_factor;
		child.transfer_qty=detail.stock_qty;
...
function getTargetWarehouse(workOrder, warehouse_list, wipWarehouse, detail) {
	// get the warehouse
...
function getContainerDetails(detail, sourceWarehouse, targetWarehouse, bomQuantity, fgCompletedQty, noOfInputs, container_used, work_order) {
	// get the containers
    return new Promise((resolve, reject) => {
        frappe.call({
            ...
            args: {
                ...
                qty: ((detail.stock_qty / bomQuantity) * fgCompletedQty) / noOfInputs,
                container_used: container_used,
                uom: detail.uom,
...
function addRowToForm(frm, detail, row, container_used, bomQuantity, add_to) {
	// this method used to set the items
		const child = frm.add_child(add_to);
		container_used.push(row.container_no);
		child.s_warehouse=row.s_warehouse;
		child.t_warehouse=row.t_warehouse;
		child.item_code=detail.item_code;
		child.item_name=detail.item_name;
		child.required_qty=(detail.stock_qty/bomQuantity)*frm.doc.fg_completed_qty/detail.no_of_inputs;
		child.basic_rate=detail.rate;
		...
		let available_qty_use="";
		let available_qty="";
		for (let i = 0; i < row.container_no.length; i++) {
			container_no=container_no+String(row.container_no[i])+","
			available_qty=available_qty+String(row.primary_available_qty[i])+","
			available_qty_use=available_qty_use+String(row.primary_available_qty_used[i])+","
			qty=qty + row.qty_in_bom_uom[i]
		}
		child.containers=container_no
		child.qty=qty
		child.available_qty=available_qty
		...
		child.available_qty_use=available_qty_use
    // Assign other values from containerDetails and detail
    // ...

    frm.refresh_field(add_to);
}
