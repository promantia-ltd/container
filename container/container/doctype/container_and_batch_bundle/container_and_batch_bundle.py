...
# from ntpt_erpnext_app.ntpt_erpnext_app.container_batch_bundle import BatchNoValuation,ContainerNoValuation
...
# 		if not (self.is_containerized and self.type_of_transaction == "Outward"):
# 			return

# 		serial_nos = [d.container for d in self.entries if d.container]
...
# 			if data.container not in serial_nos:
# 				continue

# 			serial_no_warehouse[data.container] = data.warehouse
...
# 					f"Container No {bold(serial_no)} is not present in the warehouse {bold(self.warehouse)}.",
...
# 		if not (self.is_containerized and self.type_of_transaction == "Inward"):
# 			return
...
# 		for data in available_serial_nos:
# 			if data.container in serial_nos:
# 				self.throw_error_message(
# 					f"Container No {bold(data.container)} is already present in the warehouse {bold(data.warehouse)}.",
...
# 			d.qty if self.is_containerized and abs(d.qty) > 1 else abs(d.qty) if d.qty else 0
...
# 		return [d.container for d in self.entries if d.container]

# 	def set_incoming_rate_for_outward_transaction(self, row=None, save=False):
# 		sle = self.get_sle_for_outward_transaction()

# 		if self.is_containerized:
# 			sn_obj = ContainerNoValuation(
# 				sle=sle,
# 				item_code=self.item_code,
...
# 			available_qty = 0
# 			if self.is_containerized:
# 				d.incoming_rate = abs(sn_obj.serial_no_incoming_rate.get(d.container, 0.0))
# 			else:
...
# 				"item_code": self.item_code,
# 				"warehouse": self.warehouse,
# 				"custom_container_and_batch_bundle": self.name,
# 				"actual_qty": self.total_qty,
# 				"company": self.company,
# 				"serial_nos": [row.container for row in self.entries if row.container],
...
# 		if not (self.docstatus == 1 and self.voucher_type == "Delivery Note" and self.is_containerized):
# 			return
...
# 		sn_table = frappe.qb.DocType("Container")
# 		(
...
# 		parent = frappe.qb.DocType("Container and Batch Bundle")
# 		child = frappe.qb.DocType("Container and Batch Entry")
...
# 			)
# 			.where(
# 				(child.container.isin(serial_nos))
...
# 		if future_entries:
# 			msg = """The container has been used in the future
...
# 				msg += f"<li>{d.container} in {get_link_to_form(d.voucher_type, d.voucher_no)}</li>"
# 			msg += "</li></ul>"

# 			title = "Container No Exists In Future Transaction(s)"
...
# 			self.throw_error_message(
# 				f"Total quantity {abs(self.total_qty)} in the Container and Batch Bundle {bold(self.name)} does not match with the quantity {abs(row.get(qty_field))} for the Item {bold(self.item_code)} in the {self.voucher_type} # {self.voucher_no}"
# 			)

# 	def set_is_outward(self):
# 		for row in self.entries:
# 			if self.type_of_transaction == "Outward" and float(row.qty) > 0:
# 				row.qty *= -1
# 			elif self.type_of_transaction == "Inward" and float(row.qty) < 0:
# 				row.qty *= -1
...
# 		if not (self.is_containerized and self.entries):
# 			return
...
# 		if self.item_code and not self.is_containerized and not self.has_batch_no:
# 			msg = f"The Item {self.item_code} does not have Container No or Batch No"
# 			frappe.throw(_(msg))
...
# 		for row in self.entries:
# 			if row.container:
# 				serial_nos.append(row.container)

# 			if row.batch_no and not row.container:
# 				batch_nos.append(row.batch_no)

# 			if row.container and row.batch_no and self.type_of_transaction == "Outward":
# 				serial_batches.setdefault(row.container, row.batch_no)
...
# 				self.throw_error_message(
# 					f"Container No {bold(serial_no)} does not belong to Batch No {bold(batch_no)}"
# 				)
...
# 			self.throw_error_message(
# 				f"Container Nos {bold(incorrect_serial_nos)} does not belong to Item {bold(self.item_code)}"
# 			)
...
# 				if value > 1:
# 					self.throw_error_message(f"Duplicate Container No {key} found")
...
# 		self.delink_custom_container_and_batch_bundle()
# 		self.clear_table()

# 	def delink_custom_container_and_batch_bundle(self):
# 		self.voucher_no = None

# 		sles = frappe.get_all("Stock Ledger Entry", filters={"custom_container_and_batch_bundle": self.name})

# 		for sle in sles:
# 			frappe.db.set_value("Stock Ledger Entry", sle.name, "custom_container_and_batch_bundle", None)
...
# 		or_filters = {"custom_container_and_batch_bundle": self.name}

# 		fields = ["name", "custom_container_and_batch_bundle"]
# 		if self.voucher_type == "Stock Reconciliation":
# 			fields = ["name", "current_custom_container_and_batch_bundle", "custom_container_and_batch_bundle"]
# 			or_filters["current_custom_container_and_batch_bundle"] = self.name

# 		elif self.voucher_type == "Purchase Receipt":
# 			fields = ["name", "rejected_custom_container_and_batch_bundle", "custom_container_and_batch_bundle"]
# 			or_filters["rejected_custom_container_and_batch_bundle"] = self.name
...
# 			if voucher.get("current_custom_container_and_batch_bundle"):
# 				frappe.db.set_value(self.child_table, voucher.name, "current_custom_container_and_batch_bundle", None)
# 			elif voucher.get("rejected_custom_container_and_batch_bundle"):
# 				frappe.db.set_value(self.child_table, voucher.name, "rejected_custom_container_and_batch_bundle", None)

# 			frappe.db.set_value(self.child_table, voucher.name, "custom_container_and_batch_bundle", None)

# 	def delink_reference_from_batch(self):
# 		batches = frappe.get_all(
# 			"Batch",
# 			fields=["name"],
# 			filters={"reference_name": self.name, "reference_doctype": "Container and Batch Bundle"},
# 		)
...
# 	filename = "container_and_batch_bundle"
...
# 	container, batch_nos = [], []
# 	container, batch_nos = get_serial_batch_from_csv(item_code, file_path)
...
# 		reader = csv.reader(f)
# 		container, batch_nos = parse_csv_file_to_get_serial_batch(reader)

# 	if container:
# 		make_container(item_code, container)

# 	if batch_nos:
# 		make_batch_nos(item_code, batch_nos)

# 	return container, batch_nos


# def parse_csv_file_to_get_serial_batch(reader):
# 	is_containerized, has_batch_no = False, False
# 	container = []
# 	batch_nos = []

# 	for index, row in enumerate(reader):
# 		if index == 0:
# 			is_containerized = row[0] == "Container"
...
# 		if is_containerized or (is_containerized and has_batch_no):
# 			_dict = {"container": row[0], "qty": row[2]}
...
# 		make_container(item_code, serial_nos)

# 	return serial_nos, batch_nos


# def make_container(item_code, container):
# 	item = frappe.get_cached_value("Item", item_code, ["description", "item_code","stock_uom","item_name"], as_dict=1)

# 	# containers = [d.get("container") for d in container if d.get("container")]
...
# 	frappe.db.bulk_insert("Container", fields=fields, values=set(container_details))

# 	frappe.msgprint(_("Containers are created successfully"))
...
# 		or_filters={"is_containerized": 1, "has_batch_no": 1},
...
# 	return frappe.get_all(
# 		"Container and Batch Bundle",
# 		fields=[
# 			"`tabContainer and Batch Bundle`.`name`",
# 			"`tabContainer and Batch Bundle`.`item_code`",
# 			"`tabContainer and Batch Entry`.`qty`",
# 			"`tabContainer and Batch Entry`.`warehouse`",
# 			"`tabContainer and Batch Entry`.`batch_no`",
# 			"`tabContainer and Batch Entry`.`container`",
# 		],
# 		filters=filters,
# 		order_by="`tabContainer and Batch Entry`.`idx`",
# 	)
...
# 	filters = [
# 		["Container and Batch Bundle", "is_cancelled", "=", 0],
# 	]

# 	if item_code:
# 		filters.append(["Container and Batch Bundle", "item_code", "=", item_code])

# 	if not docstatus:
# 		docstatus = [0, 1]

# 	if isinstance(docstatus, list):
# 		filters.append(["Container and Batch Bundle", "docstatus", "in", docstatus])
# 	else:
# 		filters.append(["Container and Batch Bundle", "docstatus", "=", docstatus])

# 	# if voucher_no:
# 	# 	filters.append(["Container and Batch Bundle", "voucher_no", "=", voucher_no])

# 	if name:
# 		if isinstance(name, list):
# 			filters.append(["Container and Batch Entry", "parent", "in", name])
# 		else:
# 			filters.append(["Container and Batch Entry", "parent", "=", name])

# 	return filters


# @frappe.whitelist()
# def add_container_batch_ledgers(entries, child_row, doc, warehouse) -> object:
...
# 	if frappe.db.exists("Container and Batch Bundle", child_row.custom_container_and_batch_bundle):
# 		doc = update_container_batch_no_ledgers(entries, child_row, parent_doc, warehouse)
# 	else:
# 		doc = create_container_batch_no_ledgers(entries, child_row, parent_doc, warehouse)

# 	return doc


# def create_container_batch_no_ledgers(entries, child_row, parent_doc, warehouse=None) -> object:
...
# 	doc = frappe.get_doc(
# 		{
# 			"doctype": "Container and Batch Bundle",
...
# 	frappe.db.set_value(child_row.doctype, child_row.name, "custom_container_and_batch_bundle", doc.name)

# 	frappe.msgprint(_("Container and Batch Bundle created"), alert=True)

# 	return doc


# def update_container_batch_no_ledgers(entries, child_row, parent_doc, warehouse=None) -> object:
# 	doc = frappe.get_doc("Container and Batch Bundle", child_row.container_and_batch_bundle)
...
# 	frappe.msgprint(_("Container and Batch Bundle updated"), alert=True)
...
# 	serial_batch_table = frappe.qb.DocType("Container and Batch Entry")

# 	query = (
# 		frappe.qb.from_(sle_table)
# 		.inner_join(serial_batch_table)
# 		.on(sle_table.custom_container_and_batch_bundle == serial_batch_table.parent)
# 		.select(
# 			serial_batch_table.container,
...
# 		query = query.where(serial_batch_table.container.isin(kwargs.serial_nos))
...
# 	kwargs = frappe._dict(kwargs)
# 	if cint(kwargs.is_containerized):
# 		return get_available_serial_nos(kwargs)

# 	elif cint(kwargs.custom_has_batch):
...
# 	fields = ["name as container", "warehouse","primary_available_qty as qty"]
...
# 	for d in data:
# 		if d.custom_container_and_batch_bundle:
# 			sns = get_serial_nos_from_bundle(d.custom_container_and_batch_bundle, kwargs.get("serial_nos", []))
# 			if d.actual_qty > 0:
...
# 		elif d.container:
# 			sns = get_serial_nos(d.container)
# 			if d.actual_qty > 0:
# 				serial_nos.update(sns)
# 			else:
# 				serial_nos.difference_update(sns)
...
# 	"""Returns a list of `Container` reserved in POS Invoice and Stock Reservation Entry."""
...
# 		ignore_serial_nos.append(d.container)

# 	returned_serial_nos = []
# 	for pos_invoice in pos_invoices:
# 		if pos_invoice.container:
# 			ignore_serial_nos.extend(get_serial_nos(pos_invoice.container))
...
# 	"""Returns a list of `Container` reserved in Stock Reservation Entry."""

# 	sre = frappe.qb.DocType("Stock Reservation Entry")
# 	sb_entry = frappe.qb.DocType("Container and Batch Entry")
# 	query = (
# 		frappe.qb.from_(sre)
# 		.inner_join(sb_entry)
# 		.on(sre.name == sb_entry.parent)
# 		.select(sb_entry.container)
# 		.where(
# 			(sre.docstatus == 1)
# 			& (sre.item_code == kwargs.item_code)
# 			& (sre.reserved_qty >= sre.delivered_qty)
# 			& (sre.status.notin(["Delivered", "Cancelled"]))
# 			& (sre.reservation_based_on == "Container and Batch")
# 		)
# 	)
...
# 	batch_ledger = frappe.qb.DocType("Container and Batch Entry")
# 	batch_table = frappe.qb.DocType("Batch")

# 	query = (
# 		frappe.qb.from_(stock_ledger_entry)
# 		.inner_join(batch_ledger)
# 		.on(stock_ledger_entry.custom_container_and_batch_bundle == batch_ledger.parent)
...
# 		if row.container:
# 			child_row["serial_nos"].append(row.container)
...
# 	bundle_table = frappe.qb.DocType("Container and Batch Bundle")
# 	serial_batch_table = frappe.qb.DocType("Container and Batch Entry")
...
# 			stock_ledger_entry.container,
# 			stock_ledger_entry.custom_container_and_batch_bundle,
# 		)
# 		.where((stock_ledger_entry.is_cancelled == 0))
# 	)
...
# @frappe.whitelist()
# def update_voucher_no(name):
# 	purchase_receipt= frappe.db.get_all("Purchase Receipt Item",filters={"custom_container_and_batch_bundle":name},fields=["parent","name"],order_by="creation DESC")
# 	if purchase_receipt:
# 		frappe.db.set_value("Container and Batch Bundle",name,"voucher_no",purchase_receipt[0].parent)
# 		frappe.db.set_value("Container and Batch Bundle",name,"voucher_detail_no",purchase_receipt[0].name)