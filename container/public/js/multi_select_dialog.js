frappe.ui.form.MultiSelectDialog = class MultiSelectDialog {
	...
	bind_events() {
		...
		this.$parent.find('[data-fieldtype="Data"]').on("input", () => {
			var $this = $(this);
			clearTimeouzt($this.data("timeout"));
			$this.data(
				"timeout",
				setTimeout(function () {
					frappe.flags.auto_scroll = false;
	...
	async get_results() {
		...
		if (res.values.length) {
			res.values.forEach((result) => {
				if (this.doctype=="Container" && this.selected_all_containers.includes(result.name)){
                    result.checked = 0;
                    result.selected="Yes";
                    this.results.push(result);
                    }
            	else if (this.doctype=="Container"){
                    result.checked = 0;
                    result.selected="No";
                    this.results.push(result);
                    }
