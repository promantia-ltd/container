container.ContainerBatchPackageSelector = class ContainerNoBatchBundleUpdate {
	constructor(frm, item, callback) {
		this.frm = frm;
		this.item = item;
		console.log("___________Custom1________________")
		this.qty = item.qty;
		this.callback = callback;
		this.bundle =  this.item.custom_container_and_batch_bundle;
	...
	make() {
		let label = this.item?.is_containerized ? __('Container') : __('Batch Nos');
		let primary_label = this.bundle
			? __('Update') : __('Add');

		if (this.item?.is_containerized && this.item?.batch_no) {
			label = __('Container / Batch Nos');
		}
	...
	get_dialog_fields() {
		...
		if (this.item.is_containerized) {
			fields.push({
				fieldtype: 'Data',
				fieldname: 'scan_serial_no',
				label: __('Scan Container No'),
				get_query: () => {
					return {
		...
		if (this.item.custom_has_batch && this.item.is_containerized) {
			fields.push({
				fieldtype: 'Column Break',
			});
		}

		if (this.item.custom_has_batch) {
			fields.push({
				fieldtype: 'Data',
	...
	get_attach_field() {
		...
		if (this.item?.is_containerized && this.item?.custom_has_batch) {
			label = __('Container / Batch Nos');
		}
	...
	download_csv_file() {
		let csvFileData = ['Container'];

		if (this.item.is_containerized && this.item.custom_has_batch) {
			csvFileData = ['Container', 'Batch No', 'Quantity'];
		} else if (this.item.custom_has_batch) {
			csvFileData = ['Batch No', 'Quantity'];
		}

		const method = `/api/method/container.container.doctype.container_and_batch_bundle.container_and_batch_bundle.download_blank_csv_template?content=${encodeURIComponent(JSON.stringify(csvFileData))}`;
	...
	upload_csv_file() {
		...
		frappe.call({
			method: 'container.container.doctype.container_and_batch_bundle.container_and_batch_bundle.upload_csv_file',
			args: {
				item_code: this.item.item_code,
				file_path: file_path
			},
			callback: (r) => {
				if (r.message.container && r.message.container.length) {
					this.set_data(r.message.container);
	...
	get_auto_data() {
		...
		if (qty) {
			frappe.call({
				method: 'container.container.doctype.container_and_batch_bundle.container_and_batch_bundle.get_auto_data',
				args: {
					item_code: this.item.item_code,
					warehouse: this.item.warehouse || this.item.s_warehouse,
					is_containerized: this.item.is_containerized,
					has_batch_no: this.item.custom_has_batch,
					qty: qty,
					based_on: based_on
				},
				callback: (r) => {
	...
	update_bundle_entries() {
		...
		if (entries && !entries.length || !entries) {
			frappe.throw(__('Please add atleast one Container / Batch No'));
		}

		frappe.call({
			method: 'container.container.doctype.container_and_batch_bundle.container_and_batch_bundle.add_container_batch_ledgers',
			args: {
				entries: entries,
				child_row: this.item,
				doc: this.frm.doc,
	...
	edit_full_form() {
		let bundle_id = this.item.custom_container_and_batch_bundle
		if (!bundle_id) {
			let _new = frappe.model.get_new_doc(
				"Container and Batch Bundle", null, null, true
			);

			_new.item_code = this.item.item_code;
			_new.warehouse = this.get_warehouse();
			_new.is_containerized = this.item.is_containerized;
			_new.has_batch_no = this.item.custom_has_batch;
		...
		frappe.set_route("Form", "Container and Batch Bundle", bundle_id);
		this.dialog.hide();
	}
	...
	render_data() {
		if (!this.frm.is_new() && this.bundle) {
			frappe.call({
				method: 'container.container.doctype.container_and_batch_bundle.container_and_batch_bundle.get_serial_batch_ledgers',
				args: {
					item_code: this.item.item_code,
					name: this.bundle,
